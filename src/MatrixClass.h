...
namespace stumpsmatrix {
    ...
    public:
        ...
        arma::vec compute_Xb(const arma::vec& b, const arma::vec& X_avg) {
            arma::vec res = X.cols(include_linear) * (b % col_scale_factors) - arma::sum(b % X_avg);
            return(res);
        }
        ...
        arma::vec compute_X2b(const arma::vec& b, const arma::vec& X_avg) {
            arma::vec b_X_avg = b % X_avg;
            arma::vec res = X2 * b - 2 * X.cols(include_linear) * (b_X_avg % col_scale_factors) + arma::sum(X_avg % b_X_avg);
            return(res);
        }
    ...
    public:
        ...
        arma::vec compute_Xb(const arma::vec& b, const arma::vec& X_avg) {
            arma::vec res = X * b - arma::sum(b % X_avg);
            return(res);
        }
        ...
        arma::vec compute_X2b(const arma::vec& b, const arma::vec& X_avg) {
            arma::vec b_X_avg = b % X_avg;
            arma::vec res = X2 * b - 2 * X * (b_X_avg) + arma::sum(X_avg % b_X_avg);
            return(res);
        }
    ...
    inline std::array<arma::uvec, 2> get_buckets_and_counts(const arma::vec& t, const arma::uvec& order_t, const arma::vec& cuts) {
        arma::uvec counts = arma::zeros<arma::uvec>(cuts.size() + 1);
    ...
    public:
        ...
        arma::vec compute_X2b(const arma::vec& b, const arma::vec& X_avg) {
            ...
            arma::vec z = arma::zeros(X_avg.n_rows);
            arma::vec res = compute_Xb(b2, z) + arma::sum(X_avg % X_avg % b);
            return(res);
        }
    ...
    public:
        ...
        StumpsMatrix(arma::mat& X, Rcpp::Nullable<Rcpp::IntegerVector>& _include_linear, Rcpp::Nullable<Rcpp::IntegerVector>& _include_stumps, unsigned int num_cuts, bool use_quants, unsigned int scale_X, unsigned int _ncores) {
            ...
            #endif
            {
                ...
                size_t tnum = omp_get_num_threads();
                ...
                if (any_incl_lin) {
                    if (tid == 0) {
                        ...
                        for (size_t i = (tid - 1); i < which_incl_stumps.size(); i += (tnum - 1)) {
                            // blocks[i] = TFGMatrixBlock(X.col(which_incl_stumps[i]), cuts[which_incl_stumps[i]]);
                            blocks[i + any_incl_lin].reset(new TFGMatrixBlock(X.col(which_incl_stumps[i]), num_cuts, use_quants));
                        }
                    }
                    ...
                    for (size_t i = tid; i < which_incl_stumps.size(); i += tnum) {
                        // blocks[i] = TFGMatrixBlock(X.col(which_incl_stumps[i]), cuts[which_incl_stumps[i]]);
                        blocks[i + any_incl_lin].reset(new TFGMatrixBlock(X.col(which_incl_stumps[i]), num_cuts, use_quants));
                    }
                }
        ...
        StumpsMatrix(arma::mat& X, Rcpp::XPtr<StumpsMatrix> SM_in) {
            #if defined(_OPENMP)
            omp_set_num_threads(SM_in.get()->ncores);
            #endif
            ncores = omp_get_num_threads();
        ...
        arma::vec compute_Xb(const arma::vec& b, const arma::vec& X_avg) {
            arma::vec Xb = arma::zeros(nrow);
            #if defined(_OPENMP)
            #pragma omp parallel reduction(+:Xb) shared(blocks, any_incl_lin, X_avg)
            #endif
            {
                size_t tid = omp_get_thread_num();
                size_t tnum = omp_get_num_threads();
                ...
                if (any_incl_lin) {
                    if (tid == 0) {
                        if (tnum == 1) {
                            for (size_t i = 0; i < blocks.size(); i++) {
                                Xb += blocks[i].get()->compute_Xb(b.rows(blocks[i].get()->col_offset, blocks[i].get()->col_offset + blocks[i].get()->ncol - 1), X_avg.rows(blocks[i].get()->col_offset, blocks[i].get()->col_offset + blocks[i].get()->ncol - 1));
                            }
                        }
                        else {
                            Xb += blocks[0].get()->compute_Xb(b.rows(blocks[0].get()->col_offset, blocks[0].get()->col_offset + blocks[0].get()->ncol - 1), X_avg.rows(blocks[0].get()->col_offset, blocks[0].get()->col_offset + blocks[0].get()->ncol - 1));
                        }
                    }
                    else {
                        for (size_t i = tid; i < blocks.size(); i += (tnum - 1)) {
                            Xb += blocks[i].get()->compute_Xb(b.rows(blocks[i].get()->col_offset, blocks[i].get()->col_offset + blocks[i].get()->ncol - 1), X_avg.rows(blocks[i].get()->col_offset, blocks[i].get()->col_offset + blocks[i].get()->ncol - 1));
                        }
                    }
                }
                else {
                    for (size_t i = tid; i < blocks.size(); i += tnum) {
                        Xb += blocks[i].get()->compute_Xb(b.rows(blocks[i].get()->col_offset, blocks[i].get()->col_offset + blocks[i].get()->ncol - 1), X_avg.rows(blocks[i].get()->col_offset, blocks[i].get()->col_offset + blocks[i].get()->ncol - 1));
                    }
                }
        ...
        */
        ...
        arma::vec compute_Xty(const arma::vec& y, const arma::vec& X_avg) {
            arma::vec Xty = arma::zeros(ncol);
            #if defined(_OPENMP)
            #pragma omp parallel reduction(+:Xty) shared(blocks, any_incl_lin, X_avg)
            #endif
            {
                size_t tid = omp_get_thread_num();
                size_t tnum = omp_get_num_threads();
                ...
                if (any_incl_lin) {
                    if (tid == 0) {
                        if (tnum == 1) {
                            ...
                            Xty.rows(blocks[0].get()->col_offset, blocks[0].get()->col_offset + blocks[0].get()->ncol - 1) = blocks[0].get()->compute_Xty(y, X_avg.rows(blocks[0].get()->col_offset, blocks[0].get()->col_offset + blocks[0].get()->ncol - 1));
                        }
                    }
                    else {
                        for (size_t i = tid; i < blocks.size(); i += (tnum - 1)) {
        ...
        arma::vec compute_Xty(const arma::vec& y, const arma::vec& X_avg) {
            arma::vec Xty = arma::zeros(ncol);
            #if defined(_OPENMP)
            #pragma omp parallel for reduction(+:Xty) schedule(dynamic)
            #endif
            for (size_t i = 0; i < blocks.size(); i++) {
                Xty.rows(blocks[i].get()->col_offset, blocks[i].get()->col_offset + blocks[i].get()->ncol - 1) = blocks[i].get()->compute_Xty(y, X_avg.rows(blocks[i].get()->col_offset, blocks[i].get()->col_offset + blocks[i].get()->ncol - 1));
            }
        ...
        */
        ...
        arma::vec compute_X2b(const arma::vec& b, const arma::vec& X_avg) {
            arma::vec X2b = arma::zeros(nrow);
            #if defined(_OPENMP)
            #pragma omp parallel reduction(+:X2b) shared(blocks, any_incl_lin, X_avg)
            #endif
            {
                size_t tid = omp_get_thread_num();
                size_t tnum = omp_get_num_threads();
                ...
                if (any_incl_lin) {
                    if (tid == 0) {
                        if (tnum == 1) {
                            for (size_t i = 0; i < blocks.size(); i++) {
                                X2b += blocks[i].get()->compute_X2b(b.rows(blocks[i].get()->col_offset, blocks[i].get()->col_offset + blocks[i].get()->ncol - 1), X_avg.rows(blocks[i].get()->col_offset, blocks[i].get()->col_offset + blocks[i].get()->ncol - 1));
                            }
                        }
                        else {
                            X2b += blocks[0].get()->compute_X2b(b.rows(blocks[0].get()->col_offset, blocks[0].get()->col_offset + blocks[0].get()->ncol - 1), X_avg.rows(blocks[0].get()->col_offset, blocks[0].get()->col_offset + blocks[0].get()->ncol - 1));
                        }
                    }
                    else {
                        for (size_t i = tid; i < blocks.size(); i += (tnum - 1)) {
        ...
        */
        ...
        arma::vec compute_X2ty(const arma::vec& y, const arma::vec& X_avg) {
            arma::vec X2ty = arma::zeros(ncol);
            #if defined(_OPENMP)
            #pragma omp parallel reduction(+:X2ty) shared(blocks, any_incl_lin, X_avg)
            #endif
            {
                size_t tid = omp_get_thread_num();
                size_t tnum = omp_get_num_threads();
                ...
                if (any_incl_lin) {
                    if (tid == 0) {
                        if (tnum == 1) {
                            ...
                            X2ty.rows(blocks[0].get()->col_offset, blocks[0].get()->col_offset + blocks[0].get()->ncol - 1) = blocks[0].get()->compute_X2ty(y, X_avg.rows(blocks[0].get()->col_offset, blocks[0].get()->col_offset + blocks[0].get()->ncol - 1));
                        }
                    }
                    else {
                        for (size_t i = tid; i < blocks.size(); i += (tnum - 1)) {
        ...
        arma::vec compute_X2ty(const arma::vec& y, const arma::vec& X_avg) {
            arma::vec X2ty = arma::zeros(ncol);
            #if defined(_OPENMP)
            #pragma omp parallel for reduction(+:X2ty) schedule(dynamic)
            #endif
            for (size_t i = 0; i < blocks.size(); i++) {
                X2ty.rows(blocks[i].get()->col_offset, blocks[i].get()->col_offset + blocks[i].get()->ncol - 1) = blocks[i].get()->compute_X2ty(y, X_avg.rows(blocks[i].get()->col_offset, blocks[i].get()->col_offset + blocks[i].get()->ncol - 1));
            }
        ...
        */
        ...
        arma::mat compute_X_avg_tau_no_V_nu(const arma::vec& w, const arma::vec& inv_sigma2, const arma::vec& Y_cent) {
            arma::mat res = arma::zeros(ncol, 3);
            #if defined(_OPENMP)
            #pragma omp parallel reduction(+:res) shared(blocks, w, inv_sigma2, Y_cent)
            #endif
            {
                size_t tid = omp_get_thread_num();
                size_t tnum = omp_get_num_threads();
                arma::vec z(ncol, arma::fill::zeros);

                if (any_incl_lin) {
                    if (tid == 0) {
                        if (tnum == 1) {
                            for (size_t i = 0; i < blocks.size(); i++) {
                                res.col(0).rows(blocks[i].get()->col_offset, blocks[i].get()->col_offset + blocks[i].get()->ncol - 1) = blocks[i].get()->compute_Xty(w, arma::zeros<arma::vec>(blocks[i].get()->ncol));
                                res.col(1).rows(blocks[i].get()->col_offset, blocks[i].get()->col_offset + blocks[i].get()->ncol - 1) = blocks[i].get()->compute_X2ty(inv_sigma2, res.col(0).rows(blocks[i].get()->col_offset, blocks[i].get()->col_offset + blocks[i].get()->ncol - 1));
                                res.col(2).rows(blocks[i].get()->col_offset, blocks[i].get()->col_offset + blocks[i].get()->ncol - 1) = blocks[i].get()->compute_Xty(Y_cent % inv_sigma2, res.col(0).rows(blocks[i].get()->col_offset, blocks[i].get()->col_offset + blocks[i].get()->ncol - 1));
                            }
                        }
                        else {
                            res.col(0).rows(blocks[0].get()->col_offset, blocks[0].get()->col_offset + blocks[0].get()->ncol - 1) = blocks[0].get()->compute_Xty(w, arma::zeros<arma::vec>(blocks[0].get()->ncol));
                            res.col(1).rows(blocks[0].get()->col_offset, blocks[0].get()->col_offset + blocks[0].get()->ncol - 1) = blocks[0].get()->compute_X2ty(inv_sigma2, res.col(0).rows(blocks[0].get()->col_offset, blocks[0].get()->col_offset + blocks[0].get()->ncol - 1));
                            res.col(2).rows(blocks[0].get()->col_offset, blocks[0].get()->col_offset + blocks[0].get()->ncol - 1) = blocks[0].get()->compute_Xty(Y_cent % inv_sigma2, res.col(0).rows(blocks[0].get()->col_offset, blocks[0].get()->col_offset + blocks[0].get()->ncol - 1));
                        }
                    }
                    else {
                        for (size_t i = tid; i < blocks.size(); i += (tnum - 1)) {
