...
XPtr<StumpsMatrix> make_stumps_matrix_cpp(arma::mat X, arma::uvec include_linear, arma::uvec include_stumps, std::vector<arma::vec> cuts , unsigned int ncores) {
  StumpsMatrix *X_stumps = new StumpsMatrix(X, include_linear, include_stumps, cuts, ncores);
  XPtr<StumpsMatrix> X_stumps_p(X_stumps, true);
  return(X_stumps_p);
}
...
// [[Rcpp::export]]
List weighted_SER_cpp(XPtr<StumpsMatrix> xp, arma::vec Y, arma::vec sigma2, Nullable<List> init = R_NilValue, double max_lV = 0, double lin_prior_prob = 0.5) {
  if (sigma2.size() == 1) {
    sigma2 = arma::linspace(sigma2.at(0, 0), sigma2.at(0, 0), Y.size());
  }
  arma::vec inv_sigma2 = 1 / sigma2;
  ...
  if (init.isNull() || !init.as().containsElementNamed("prior_weights") || !init.as().containsElementNamed("V")) {
    ...
    prior_weights = join_cols(arma::linspace(lin_prior_prob / p_lin, lin_prior_prob / p_lin, p_lin), arma::linspace((1 - lin_prior_prob) / p_stumps, (1 - lin_prior_prob) / p_stumps, p_stumps)) / (lin_prior_prob*(p_lin > 0) + (1 - lin_prior_prob)*(p_stumps > 0));
  } else {
    lV_init = log((double)init.as()["V"]);
    prior_weights = as<arma::vec>(init.as()["prior_weights"]);
  }

  double Y_avg = sum(Y % w);
  arma::vec Y_cent = Y - Y_avg;
  arma::vec X_avg = xp.get()->compute_Xty(w, zeros(xp.get()->ncol));

  arma::vec tau_no_V = xp.get()->compute_X2ty(inv_sigma2, X_avg);
  arma::vec nu = xp.get()->compute_Xty(Y_cent / sigma2, X_avg);
  ...
  List optim_res = r_optim(Named("par") = lV_init, Named("fn") = InternalFunction(&nll), Named("tau_no_V") = tau_no_V, Named("nu") = nu, Named("sigma2") = sigma2,
  ...
  arma::vec alpha = log(prior_weights / sqrt(tau)) + (0.5 * pow(nu, 2) / tau);
  ...
  arma::vec mu2 = pow(Y_avg, 2) + 2*Y_avg*Xb_post + xp.get()->compute_X2b(beta_post_2, X_avg);
...
arma::vec predFnSusieStumps_cpp(XPtr<StumpsMatrix> xp_new, List currentFit, int moment) {
  ...
  if (moment == 1) {
    ...
    return(pow(as<double>(currentFit["Y_avg"]), 2) + 2*as<double>(currentFit["Y_avg"])*Xb_post + xp_new.get()->compute_X2b(beta_post_2, as<arma::vec>(currentFit["X_avg"])));
  } 
