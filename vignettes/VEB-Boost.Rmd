...
Define the space of a node's fit to be $\mathcal{F}$; for the variational approximation $q$ of the fit of $\mu$ encoded in $\mathcal{F}$, this space must contain information on the following:
$$
\begin{aligned}
...
- `$KL_div` $\in \mathbb{R}_+$: contains the KL-divergence from the variational approximation to the prior at the node, i.e. $D_{KL}(q || p) \quad \text{for prior distribution } \; p$.
